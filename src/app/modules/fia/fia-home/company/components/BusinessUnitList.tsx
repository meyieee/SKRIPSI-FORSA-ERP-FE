...
import { BusinessUnit } from '../core/_models'

interface BusinessUnitListProps {
  businessUnits: BusinessUnit[]
}

export const BusinessUnitList = ({ businessUnits }: BusinessUnitListProps) => {
  return (
    <KTCard className='h-100'>
      <div className='card-header border-0 pt-5'>
        <h3 className='card-title align-items-start flex-column'>
          <span className='card-label fw-bold fs-3 mb-1'>BUSINESS UNIT</span>
        </h3>
      </div>
          ...
          <table className='table align-middle table-hover table-row-dashed fs-6 gy-0 dataTable no-footer'>
            <thead>
              <tr className='text-start text-muted fw-bolder fs-7 text-uppercase gs-0'>
                ...
                <th className='min-w-150px'>Business Unit</th>
                ...
                <tr key={unit.id}>
                  ...
                  <td>
                    <div className='d-flex flex-column'>
                      <span className='fw-semibold fs-6 text-gray-800'>{unit.name}</span>
                      <span className='text-muted fs-7'>{unit.type}</span>
                    </div>
                  </td>
