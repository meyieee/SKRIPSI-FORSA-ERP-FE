...
import { cache_trainingrequest_new, cache_trainingrequest_list } from '../../../../../../../constans/CachesName'
...
import './TrainingRequestForm.scss'
...
function TrainingRequestForm({ cat, type }: Props) {
  ...
  // Use React Query to fetch training request form data
  const { data, isLoading: loading, error: queryError } = UseReactQuery<TrainingRequestFormType>({
    func: () => getTrainingRequestNew(),
    cacheName: cache_trainingrequest_new,
    enabled: true,
  })
  ...
  const onSubmit = async (values: TrainingRequestFormType, actions: any) => {
    const errs = validateTrainingRequest(values)
    if (errs.length) {
    ...
    try {
      setSubmitting(true)
      const res = await postTrainingRequest(values)
      
      // Invalidate cache after successful submission
      queryClient.invalidateQueries({ queryKey: [cache_trainingrequest_list] })
      queryClient.invalidateQueries({ queryKey: [cache_trainingrequest_new] })
      
      // Show success modal dengan requestId dan refRequestNo
      showSuccessModal(
        res.message || 'Training request submitted successfully!',
  ...
  // Handle save draft
  const onSave = async (values: TrainingRequestFormType) => {
    if (!values) return
    try {
      await saveTrainingRequestDraft(cat, type, values)
      showDraftSavedModal()
    } catch (e: any) {
      ...
      <div className="card">
        <div className="card-body">
          <div className='alert alert-danger'>
            Failed to load training request form: {String(queryError)}
          </div>
        </div>
      </div>
    )
  ...
  if (!data) {
    return (
      <div className="card">
        <div className="card-body">
          <div className='alert alert-danger'>
            Failed to load training request form data for {cat} - {type}
          </div>
        </div>
      </div>
    )
          ...
          {(formProps) => (
            <>
              <div className='card-header d-flex justify-content-between align-items-center'>
                <div>
                  <h3 className='card-title mb-2'>TRAINING REQUEST FORM</h3>
                  <HeaderSection
                ...
                <div className='row'>
                  <div className='col-12'>
                    ...
                    <TrainingDetailsSection
                      values={formProps.values.trainingDetails}
                      setFieldValue={formProps.setFieldValue}
                      getTrainingMethodOptions={getTrainingMethodOptions}
                    />
                    </Form>
                  </div>
                </div>
