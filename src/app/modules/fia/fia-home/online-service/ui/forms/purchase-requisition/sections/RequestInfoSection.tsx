...
import { PurchaseRequisitionInfo } from '../../../../core/purchase-requisition'
...
type RequestInfoSectionProps = {
  values: PurchaseRequisitionInfo
...
export default function RequestInfoSection({
  ...
  const { departmentOptions: orgDepartmentOptions } = useOrgMasterOptions(values.department)
    ...
    <FormSection title='Request Information'>
      <div className='row'>
        <div className='col-md-4'>
          <FormField
            label='Request Date'
            name='requestInfo.requisitionDate'
            value={values.requisitionDate}
            onChange={(value) => setFieldValue('requestInfo.requisitionDate', value)}
            type='date'
            required
          />
        </div>
