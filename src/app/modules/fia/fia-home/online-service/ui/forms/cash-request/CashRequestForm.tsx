...
import { cache_cashrequest_new, cache_cashrequest_list } from '../../../../../../../constans/CachesName'
import { useAuth } from '../../../../../../../modules/auth'
import { 
  getCashRequestNew,
  postCashRequest,
  saveCashRequestDraft,
  validateCashRequestAll,
...
import CashRequestDetailsSection from './sections/CashRequestDetailsSection'
import './CashRequestForm.scss'
...
function CashRequestForm({ cat, type }: Props) {
  ...
  // Use React Query to fetch cash request form data
  const { data, isLoading: loading, error: queryError } = UseReactQuery<CashRequestFormType>({
    func: () => getCashRequestNew(),
  ...
  const onSubmit = async (values: CashRequestFormType, actions: any) => {
    const errs = validateCashRequestAll(values)
    if (errs.length) {
    ...
    try {
      setSubmitting(true)
      const res = await postCashRequest(values)
      
      // Invalidate cache after successful submission
      queryClient.invalidateQueries({ queryKey: [cache_cashrequest_list] })
      queryClient.invalidateQueries({ queryKey: [cache_cashrequest_new] })
      
      // Show success modal dengan requestId dan refRequestNo
      showSuccessModal(
        res.message || 'Cash request submitted successfully!',
  ...
  // Handle save draft
  const onSave = async (values: CashRequestFormType) => {
    if (!values) return
    try {
      await saveCashRequestDraft(cat, type, values)
      showDraftSavedModal()
    } catch (e: any) {
      ...
      <div className="card">
        <div className="card-body">
          <div className='alert alert-danger'>
            Failed to load cash request form: {String(queryError)}
          </div>
        </div>
      </div>
    )
  ...
  if (!data) {
    return (
      <div className="card">
        <div className="card-body">
          <div className='alert alert-danger'>
            Failed to load cash request form data for {cat} - {type}
          </div>
        </div>
      </div>
    )
                ...
                <div className='row'>
                  <div className='col-12'>
                    <Form className='w-100'>
                      ...
                      <CashRequestDetailsSection
                        values={formProps.values.cashRequestDetails}
                        setFieldValue={formProps.setFieldValue}
                        getExpenseTypeOptions={getExpenseTypeOptions}
