...
import { jobRows } from '../dummy/jobDummy'
...
export const JobReqControlTab: React.FC = () => {
  ...
  // options
  const siteOptions = useMemo(() => uniq(jobRows.map((r) => r.siteBranch)), [])
  const deptOptions = useMemo(() => uniq(jobRows.map((r) => r.department)), [])
  const sectionOptions = useMemo(() => uniq(jobRows.map((r) => r.section)), [])
  const elementOptions = useMemo(() => uniq(jobRows.map((r) => r.element)), [])
  ...
  const rows = useMemo(() => {
    return jobRows
      ...
      .filter((r) => (filters.date ? r.reqDate === filters.date : true))
      .sort(
        (a, b) =>
          a.department.localeCompare(b.department) ||
          a.workgroup.localeCompare(b.workgroup) ||
          a.reqDate.localeCompare(b.reqDate) ||
        ...
        <table className="table table-rounded border table-row-bordered align-middle gs-4 gy-2 gx-3">
          <thead>
            <tr className="fw-bold bg-secondary">
              ...
              <th className='min-w-30px py-3'>Job Description</th>
              <th className='min-w-30px py-3'>Request By</th>
              <th className='min-w-30px py-3'>Request Date</th>
              <th className='min-w-30px py-3'>Priority</th>
              <th className='min-w-30px py-3'>Plan Start</th>
              <th className='min-w-30px py-3'>Plan Finish</th>
              <th className='min-w-30px py-3'>Job Location</th>
              ...
              (() => {
                ...
                rows.forEach((r) => {
                  if (r.department !== lastDept) {
                    out.push(renderHeaderRow(r.department.toUpperCase(), 12, 'dept'))
                    lastDept = r.department
                    lastWg = ''
                  }
                  if (r.workgroup !== lastWg) {
                    out.push(renderHeaderRow(r.workgroup, 12, 'wg'))
                    lastWg = r.workgroup
                  }
                  ...
                  out.push(
                    ...
                    <td>{n}</td>
                      ...
                      <td>{r.reqBy}</td>
                      <td>{fmtDate(r.reqDate)}</td>
                      <td>{r.priority}</td>
                      <td>{fmtDate(r.planStart)}</td>
                      <td>{fmtDate(r.planFinish)}</td>
                      <td>{r.jobLocation}</td>
                      <td>{r.costCenter}</td>
                      <td>{r.status}</td>
...
export default JobReqControlTab
