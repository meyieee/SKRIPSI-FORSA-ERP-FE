...
import { fleetRows } from '../dummy/fleetDummy'
...
export const FleetReqControlTab: React.FC = () => {
  ...
  // options
  const siteOptions = useMemo(() => uniq(fleetRows.map((r) => r.siteBranch)), [])
  const deptOptions = useMemo(() => uniq(fleetRows.map((r) => r.department)), [])
  const sectionOptions = useMemo(() => uniq(fleetRows.map((r) => r.section)), [])
  const elementOptions = useMemo(() => uniq(fleetRows.map((r) => r.element)), [])
        ...
        <table className="table table-rounded border table-row-bordered align-middle gs-4 gy-2 gx-3">
          <thead>
            <tr className="fw-bold bg-secondary">
              ...
              <th className='min-w-30px py-3'>Fleet Type-Model</th>
              <th className='min-w-30px py-3'>No. of Unit</th>
              ...
              <th className='min-w-30px py-3'>Fleet Location</th>
...
export default FleetReqControlTab
