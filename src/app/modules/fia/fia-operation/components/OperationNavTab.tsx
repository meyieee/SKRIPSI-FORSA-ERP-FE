...
import '../components/styles/operationstylenavtab.scss'
...
export const OperationNavTab: React.FC<NavTabProps> = ({
  tiles,
  className = '',
  cols,
  autoStretch = true,
  ...
  return (
    <div className={`position-relative ${className}`}>
      <div className='onavtab-grid rounded-2 shadow-sm' style={style}>
        ...
        {mainTiles.map((t) => {
          ...
          return (
            <NavLink
              key={t.to}
              to={t.to}
              className={({isActive}) => `onavtab-tile ${isActive ? 'active' : ''}`}
              style={tileStyle}
              {...(t.span ? {'data-span': true} : {})}
            >
              <div className='onavtab-title'>{t.title}</div>
              {t.sub && <div className='onavtab-sub'>({t.sub})</div>}
            </NavLink>
          )
        })}
