...
import {trainingRows, trainingHistoryMap} from '../dummy/trainingDummy'
...
type Filters = {
  ...
  section: string
  element: string
  date: string // '' | YYYY-MM-DD (trainingDate)
}

type HistoryTarget = {empNo: string; empName: string} | null

export const TrainingDevelopmentControl: React.FC = () => {
  ...
  const rows = useMemo(() => {
    return trainingRows
      ...
      .filter((r) => (filters.date ? r.trainingDate === filters.date : true))
      .sort(
        (a, b) =>
  ...
  // history rows (with manual trigger via "View")
  const historyRows = useMemo(() => {
    if (!historyFor) return []
    const rows = trainingHistoryMap[historyFor.empNo] ?? []
        ...
        <table className='table table-hover table-rounded border table-bordered table-row-gray-300 align-middle text-dark gs-4 gy-1 gx-3'>
          <thead>
            <tr className='fw-bold bg-secondary'>
              ...
              <th className='min-w-220px py-3'>Training Program</th>
              <th className='min-w-140px py-3'>Training Dates</th>
              <th className='min-w-110px py-3'>Duration</th>
              <th className='min-w-220px py-3'>Location</th>
              <th className='min-w-180px py-3'>Provider</th>
                ...
                rows.forEach((r) => {
                  ...
                  out.push(
                    <tr key={r.id}>
                      ...
                      <td>{r.jobTitle}</td>
                      <td>{r.trainingProgram}</td>
                      <td>{fmtDate(r.trainingDate)}</td>
                      <td>{r.duration}</td>
                      <td>{r.location}</td>
                      <td>{r.provider}</td>
        ...
        <div className='mt-5' ref={historyRef}>
          <div className='bg-secondary px-3 py-2 d-flex flex-row justify-content-between align-items-center rounded-top'>
            <div className='fw-semibold'>
              Training History - {historyFor.empName} - {historyFor.empNo}
            </div>
          ...
          <div className='table-responsive border border-top-0 rounded-bottom'>
            <table className='table table-sm align-middle mb-0 gs-3'>
              <thead className='table-dark'>
                <tr>
                  <th className='min-w-160px'>Training Date</th>
                  <th className='min-w-120px'>Duration</th>
                  <th className='min-w-260px'>Training Program</th>
                  <th className='min-w-260px'>Training Location</th>
                  <th className='min-w-200px'>Training Provider</th>
              ...
              <tbody>
                {historyRows.length === 0 ? (
                  ...
                  historyRows.map((h) => (
                    <tr key={h.id}>
                      <td>{fmtDate(h.trainingDate)}</td>
                      <td>{h.duration}</td>
                      <td>{h.trainingProgram}</td>
                      <td>{h.trainingLocation}</td>
                      <td>{h.trainingProvider}</td>
                      <td className='text-end'>
                        <button
                          type='button'
...
export default TrainingDevelopmentControl
